module FSharp.QuestPDF.Tests.DocsTests
...
/// The documentation file of FSharp.QuestPDF: $FSHARP_QUESTPDF_DOCS_XML when set, otherwise the file beside the
/// assembly.
let private docsXml () =
    match
        Environment.GetEnvironmentVariable "FSHARP_QUESTPDF_DOCS_XML"
        |> Option.ofObj
    with
    | Some path -> path
        ...
        [ test "every QuestPDF cref names a QuestPDF member" {
              let crefs =
                  [ for m in Regex.Matches (File.ReadAllText (docsXml ()), "cref=\"([A-Z]:QuestPDF\\.[^\"]*)\"") do
                        ...
                        if not (cref.Substring(2).StartsWith "FSharp.QuestPDF.") then
                            yield cref ]
          ...
          test "QuestPDF links are replaced by their text" {
              ...
              Expect.equal
                  (FSharp.QuestPDF.Build.QuestPdfLinks.unlink html)
          ...
          test "the interop mapping table is generated from Coverage.fs" {
              Expect.stringContains (pageSource "interop") "#load \"../tests/FSharp.QuestPDF.Tests/Coverage.fs\"" "loads Coverage.fs"
          } ]
