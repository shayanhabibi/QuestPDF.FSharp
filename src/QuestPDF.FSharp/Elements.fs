...
[<AutoOpen>]
module Elements =
    /// <summary>A horizontal line of a thickness and colour, as wide as the slot; the thickness accepts int, float, float32 (points) or Length.</summary>
    /// <remarks>
    ...
    /// <summary>A vertical line of a thickness and colour, as tall as the slot; the thickness accepts int, float, float32 (points) or Length.</summary>
    /// <remarks>
    ...
    /// <c>fun l -&gt; l.LineColor(c).LineDashPattern [| 4f; 2f |]</c>; the thickness accepts int, float, float32 (points) or Length.
    /// </summary>
    ...
    /// <summary>
    /// A vertical line of a thickness, styled by a fluent chain on the line descriptor; the thickness accepts int, float,
    /// float32 (points) or Length.
    /// </summary>
    ...
    let pageBreak: Content = fun (Slot container) -> container.PageBreak ()
    ...
    let placeholder (label: string) : Content =
        fun (Slot container) -> container.Placeholder label
...
module Image =
    ...
    let shared (image: QuestPDF.Infrastructure.Image) (option: ImageOption) : Content =
        fun (Slot container) -> container.Image image |> option |> ignore
    ...
    let fitWidth: ImageOption = fun image -> image.FitWidth ()
    ...
    let fitHeight: ImageOption = fun image -> image.FitHeight ()
    ...
    let fitArea: ImageOption = fun image -> image.FitArea ()
    ...
    let fitUnproportionally: ImageOption = fun image -> image.FitUnproportionally ()
    ...
    let original: ImageOption = fun image -> image.UseOriginalImage ()
    ...
    let dpi (value: int) : ImageOption =
        fun image -> image.WithRasterDpi value
    ...
    let quality (value: ImageCompressionQuality) : ImageOption =
        fun image -> image.WithCompressionQuality value
