...
`build.fsx` is the repository pipeline; `tools/*.fsx` or `scripts/*.fsx` are the finer-grained scripts it routes to.
...
- **Partas.TypeProvider.BuildHelper** — the `Repo` provider: `Repo.Project.*` for project and
  solution paths, `Repo.FileSystem.*` for directories, and setup a virtual directory system
  to use through `Repo.VirtualSystem.*`. Can opt-out if it interferes with agent workflows.
...
- `Repo.FileSystem` is a *live* view of the tree — a member only exists if that directory exists
  ...
  paths; build those as strings off the repository root, use a virtual file system passed as the second
  static parameter to the type provider, or the script stops compiling on a clean checkout.
